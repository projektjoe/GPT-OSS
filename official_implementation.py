...
class AttentionBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        t = self.norm(x)
        qkv = self.qkv(t)
        q = qkv[:, : self.num_attention_heads * self.head_dim].contiguous() # (n, 5120)
        k = qkv[
            :,
...
class MLPBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        t = self.norm(x)
        g = self.gate(t)
        experts = torch.topk(g, k=self.experts_per_token, dim=-1, sorted=True) #[2.046875, 1.6484375, 0.5703125, 0.52734375]
...
class TokenGenerator:
    ...
    def generate(self,
        ...
        while max_tokens == 0 or num_generated_tokens < max_tokens:
            logits = self.model(torch.as_tensor(tokens, dtype=torch.int32, device=self.device))
            if num_generated_tokens == 0:
                for i in range(len(logits)):
                    predicted_token = torch.argmax(logits[i], dim=-1).item()
                    print(f"Argmax is {predicted_token}")
            logits = logits[-1]
            if temperature == 0.0:
...
def main():
    """
    Main function to run the TokenGenerator on CPU mode with proper GPT-OSS Harmony encoding.
    """
    # Set device to CPU
    device = torch.device("cpu")
    
    # Path to your model checkpoint - update this path
    checkpoint_path = "gpt-oss-20b/original/"  # Update this to your actual checkpoint path
    ...
    print("Loading GPT-OSS tokenizer from HuggingFace...")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    
    # Load the official GPT-OSS harmony encoding
    encoding = load_harmony_encoding(HarmonyEncodingName.HARMONY_GPT_OSS)
    
    # Create conversation using Harmony format
    messages = [
    ...
    # Render conversation for completion - this gives us the proper token IDs
    ...
    print(f"Using Harmony encoding successfully!")
    print(f"Prompt tokens length: {len(prompt_tokens)}")
    
    
    # Initialize and run the model
    print("Initializing TokenGenerator on CPU...")
    ...
    for token in generator.generate(
        ...
        stop_tokens=[200002],
        temperature=0,  # Recommended temperature for GPT-OSS
        max_tokens=100,
    ...
    for i, msg in enumerate(parsed_messages):
        print(f"Message {i+1}: {msg.to_dict()}")
